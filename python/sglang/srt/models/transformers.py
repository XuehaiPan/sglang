...
# ==============================================================================
"""Inference-only HuggingFace Transformers fallback implementation."""
...
class TransformersAttention(nn.Module):
    def __init__(
        self,
...
class TransformersDecoderLayer(nn.Module):
    def __init__(
        self,
        config: PretrainedConfig,
        layer_id: int = 0,
        ...
        self.self_attn = TransformersAttention(
            config=config,
...
class TransformersModel(nn.Module):
    def __init__(
        ...
        self.layers = make_layers(
            ...
            lambda idx, prefix: TransformersDecoderLayer(
...
class TransformersForCausalLM(nn.Module):
    ...
    def _init_model(
        ...
        return TransformersModel(config, quant_config=quant_config, prefix=prefix)
    ...
    def get_weights_by_name(
        ...
        try:
            ...
            logger.error(
                f"Error getting weights by name {name} in "
                f"{self.__class__.__name__}: {get_exception_traceback()}"
            )
            return None
